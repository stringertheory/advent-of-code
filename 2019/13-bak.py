...
while True:
    try:
        ...
        if not len(outputs) % 3:
            ...
            xs = [x for x, y, t in iterutils.chunked(outputs, 3)]
            ys = [y for x, y, t in iterutils.chunked(outputs, 3)]
            ts = [t for x, y, t in iterutils.chunked(outputs, 3)]
            ...
            paddle_x = None
            ball_x = None
            for chunk in iterutils.chunked(outputs, 3):
                x, y, tile_id = chunk
                if x == -1 and y == 0:
                    print(x, y, tile_id)
                else:
                    game[y][x] = chars[tile_id]
                if tile_id == 4:
                    ball_x = x
                elif tile_id == 3:
                    paddle_x = x
